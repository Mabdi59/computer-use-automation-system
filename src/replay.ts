...
import { capabilityArtifactSchema, replayResultSchema, type CapabilityArtifact, type LLMAction, type PolicyConfig } from './core/schemas.js';
...
const validateInputs = (artifact: CapabilityArtifact, inputs: Record<string, string>): void => {
  for (const parameter of artifact.inputParameters) {
    ...
    if (parameter.pattern && value && !(new RegExp(parameter.pattern).test(value))) {
...
const hydrateAction = (
  ...
  if ('url' in hydrated && hydrated.url) {
    hydrated.url = options?.overrideNavigateUrl ?? substituteTemplate(hydrated.url, inputs);
  }
...
const failureResult = (
  ...
  overrides: Omit<Extract<ReturnType<typeof replayResultSchema.parse>, { status: 'failure' }>, 'status' | 'correlationId' | 'evidencePaths'>,
  evidencePaths: string[]
) => replayResultSchema.parse({ status: 'failure', correlationId, evidencePaths, ...overrides });
...
const detectObservationResult = (
  ...
  const failureMatrix = [
    ['Validation error', 'VALIDATION_ERROR', 'Synthetic target application reported a validation error.', 'hard-failure'],
    ['Permission denied', 'PERMISSION_DENIED', 'Synthetic target application denied access.', 'hard-failure'],
    ['Session expired', 'SESSION_EXPIRED', 'Synthetic target application expired the session.', 'recoverable'],
    ['Synthetic application error', 'APPLICATION_ERROR', 'Synthetic target application returned an application error.', 'hard-failure'],
  ] as const;
  for (const [needle, code, message, recoverability] of failureMatrix) {
    if (observation.visibleText.includes(needle)) {
      return failureResult(
        correlationId,
        {
          category: needle === 'Permission denied'
            ? 'Permission denial'
...
export const runReplay = async (params: {
  ...
  const artifact = capabilityArtifactSchema.parse(JSON.parse(await readFile(params.artifactPath, 'utf8')));
  ...
  const targets = new Map(artifact.targets.map((target) => [target.id, target]));
  const outputs: Record<string, string> = {};

  try {
    for (const step of artifact.steps) {
      const action = hydrateAction(step.action, params.inputs, {
        overrideNavigateUrl: params.targetUrl && step.action.type === 'navigate' ? params.targetUrl : undefined,
      });
      const target = 'targetId' in action && action.targetId ? targets.get(action.targetId) : undefined;
      const observationBefore = await adapter.observe(join(evidence.runDir, 'screenshots'), `${step.id}-before`);
      await evidence.appendEvent({ stepId: step.id, phase: 'before', observation: sanitizeObject(observationBefore), action });
      ...
      const policyDecision = evaluatePolicy(params.policy, action, observationBefore.url, target?.name);
      if (!policyDecision.allowed) {
        if (policyDecision.risk === 'irreversible' && params.handoffManager) {
          ...
          await adapter.perform({ type: 'wait', ms: 100, rationale: 'flush human audit events' });
      ...
      while (attempt < step.retryPolicy.maxAttempts) {
        try {
          const execution = await adapter.perform(action, target);
          ...
          attempt += 1;
          await evidence.appendEvent({ stepId: step.id, phase: 'retry', attempt, error: executionError.message });
          if (attempt < step.retryPolicy.maxAttempts) {
            await adapter.perform({ type: 'wait', ms: 500, rationale: 'bounded retry wait' });
          }
        }
      }
      if (executionError) {
      ...
      let observationAfter = await adapter.observe(join(evidence.runDir, 'screenshots'), `${step.id}-after`);
      for (let checkpointAttempt = 1; checkpointAttempt < step.retryPolicy.maxAttempts; checkpointAttempt += 1) {
        const checkpointSatisfied =
          (!step.expectedPageState.urlIncludes || observationAfter.url.includes(step.expectedPageState.urlIncludes)) &&
          (!step.expectedPageState.textIncludes || observationAfter.visibleText.includes(step.expectedPageState.textIncludes));
        if (checkpointSatisfied) {
          break;
        }
        await evidence.appendEvent({ stepId: step.id, phase: 'checkpoint-retry', attempt: checkpointAttempt });
        await adapter.perform({ type: 'wait', ms: 500, rationale: 'checkpoint retry wait' });
        observationAfter = await adapter.observe(join(evidence.runDir, 'screenshots'), `${step.id}-after-retry-${checkpointAttempt}`);
      }
      await evidence.appendEvent({ stepId: step.id, phase: 'after', observation: sanitizeObject(observationAfter), outputs });
      ...
      if (step.expectedPageState.urlIncludes && !observationAfter.url.includes(step.expectedPageState.urlIncludes)) {
        const result = failureResult(
          evidence.correlationId,
          {
      ...
      if (step.expectedPageState.textIncludes && !observationAfter.visibleText.includes(step.expectedPageState.textIncludes)) {
        const result = failureResult(
          evidence.correlationId,
          {
            category: 'Checkpoint failure',
    ...
    const result = replayResultSchema.parse({ status: 'success', outputs, correlationId: evidence.correlationId });
    await evidence.writeJson('result.json', result);
    await evidence.writeManifest('replay', 'none');
    return result;
  } catch (error) {
