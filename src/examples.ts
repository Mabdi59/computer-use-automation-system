...
export const buildExampleMemberBalanceArtifact = (
  baseUrl: string
): CapabilityArtifact => {
  const withoutIntegrity: Omit<CapabilityArtifact, 'integrity'> = {
    ...
    description:
      'Looks up synthetic member details and extracts the savings balance from the legacy frame.',
    revision: 1,
    vendor: {
    ...
    targets: [
      ...
      },
      {
        ...
        stableReason:
          'Stable label rendered by the legacy search form inside a same-origin iframe.',
        risk: 'safe',
        priority: [
    ...
    steps: [
      {
        id: 'step-1',
        description: 'Open application shell',
        action: {
          type: 'navigate',
          url: `${baseUrl}/`,
          rationale: 'artifact step',
        },
        risk: 'safe',
        timeoutMs: 5000,
      ...
      },
      {
        ...
        action: {
          type: 'click',
          targetId: 'member-search-link',
          rationale: 'artifact step',
        },
        risk: 'safe',
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 2 },
        expectedPageState: { textIncludes: 'Member ID' },
      },
      {
        id: 'step-3',
      ...
      {
        id: 'step-5',
        description: 'Verify details page',
        action: {
          type: 'assert',
          targetId: 'savings-balance-label',
        ...
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 2 },
        expectedPageState: { textIncludes: 'Savings Balance' },
        checkpoint: 'member-details-loaded',
      },
      {
        id: 'step-6',
        ...
        action: {
          type: 'extract',
          targetId: 'savings-balance-value',
        ...
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 1 },
        expectedPageState: { textIncludes: '$1,234.56' },
      },
    ],
    ...
    businessOutcomeDetectors: [
      {
        ...
        description: 'Search completed without a synthetic member match.',
        matchText: 'No synthetic member matched',
      },
    ],
    recoverableConditionHandlers: [
      { category: 'Transient timeout', strategy: 'retry', maxAttempts: 2 },
    ],
    redaction: {
  ...
  const provisionalArtifact = capabilityArtifactSchema.parse({
    ...withoutIntegrity,
    integrity: { algorithm: 'sha256', checksum: 'pending' },
  });
  ...
  return capabilityArtifactSchema.parse({
    ...
    integrity: {
      algorithm: 'sha256',
      checksum: computeArtifactChecksum(
...
export const buildOpenSubAccountArtifact = (
  baseUrl: string
): CapabilityArtifact => {
  const withoutIntegrity: Omit<CapabilityArtifact, 'integrity'> = {
    schemaVersion: '1.0.0',
    ...
    description:
      'Navigates a synthetic operator to the review page and blocks final confirmation.',
    revision: 1,
    vendor: {
      name: 'Synthetic Credit Union',
      application: 'Legacy Servicing Portal',
      ...
      outputs: [
        {
          name: 'reviewState',
          type: 'string',
          description: 'Review page text',
        },
      ],
      allowedBusinessOutcomes: [],
    },
    ...
    targets: [
      ...
      },
      {
        ...
        priority: [
          {
            kind: 'text',
            text: 'Synthetic only. No real transaction will be executed.',
            frameName: 'legacy-app-frame',
          },
        ],
      },
    ...
    steps: [
      ...
      {
        id: 'step-5',
        description: 'Open sub-account flow',
        action: {
          type: 'click',
          targetId: 'open-sub-account-link',
          rationale: 'artifact step',
        },
        risk: 'safe',
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 1 },
        expectedPageState: { textIncludes: 'Open Sub-Account - Step 1' },
      },
      {
        id: 'step-6',
        ...
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 2 },
        expectedPageState: { textIncludes: 'Open Sub-Account - Review' },
      },
      {
        id: 'step-7',
        description: 'Require approval before confirm',
        action: {
          type: 'click',
          targetId: 'confirm-submission-button',
          rationale: 'artifact step',
        },
        risk: 'irreversible',
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 1 },
        expectedPageState: { textIncludes: 'Confirm Submission' },
      },
      {
        id: 'step-8',
        description: 'Capture review banner',
        action: {
          type: 'extract',
          targetId: 'review-text',
          outputKey: 'reviewState',
          rationale: 'artifact step',
        },
        risk: 'safe',
        timeoutMs: 5000,
        retryPolicy: { maxAttempts: 1 },
        expectedPageState: {
          textIncludes: 'Synthetic only. No real transaction will be executed.',
        },
      },
    ],
